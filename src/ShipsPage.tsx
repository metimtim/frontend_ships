...
const ShipsPage = () => {
    ...
    const [filteredShips, setFilteredShips] = useState(mockShips);
    ...
    const fetchShips = async () => {
        try {
            ...
            const filteredData = shipsData.filter(item => item.id_ship !== undefined);
            const parkingIdData = shipsData.find(item => item.parking_id);
            const parkingCountData = shipsData.find(item => item.count);
            ...
            const requestData = mockShips.find(item => item);
            setCurrentParkingId(requestData?.parking_id || null);
    ...
    const handleSearch = async (class_name) => {
        class_name.preventDefault();
        try {
            const response = await fetch(`/api/ships/?class_name=${inputValue}`);
            const result = await response.json();
            const filteredResult = result.filter(item => item.id_ship !== undefined);
            setFilteredShips(filteredResult);
        } catch (error) {
